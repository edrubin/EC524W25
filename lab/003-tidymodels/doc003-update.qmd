...
train_df %<>% select(-roof_matl, -exterior1st, -heating_qc, -electrical)
...
parameter_grid = expand.grid(
  penalty = c(10^seq(2, -3, length.out = 25)),
...
metric_df = fit_cv %>% collect_metrics(summarize = TRUE) %>% arrange(mean)
# fit_cv %>% collect_metrics(summarize = TRUE) %>% select(mean) %>% fndistinct()
```
